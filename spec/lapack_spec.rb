      ...
      describe "GESVD/GESDD" do 
        before :each do 
          ...
          if [:float32, :float64].include? dtype
            ...
            @a = NMatrix.new([5,6], [8.79, 9.93, 9.83, 5.45, 3.16, 6.11, 6.91, 5.04, -0.27, 7.98, -9.15, -7.93, 4.86, 4.85, 3.01, 9.57, 1.64, 8.83, 0.74, 5.8, -3.49, 4.02, 9.8, 10.0, 4.27, 9.84, 0.15, -8.99, -6.02, -5.31], dtype: dtype)
            ...
            @a = NMatrix.new([4,3], 0, dtype: dtype)
          end
        end #before :each
        ...
        it "Should equal source when product is combined 'A = USV**T'" do
          @u, @s_temp, @vt = @a.gesvd
          @s = NMatrix.new(@a.shape, 0, dtype: dtype)
          @s_temp.each_with_index do |n, i|
            @s[i,i] = n
          end
          @a.should be_within(@err*1e3).of @u.transpose.dot(@s).dot(@vt.transpose).transpose
        end   
